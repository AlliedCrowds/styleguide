...
- `release/`: Used for finalizing functionality and user testing before a new release. No new features can be added to `release` branches —only bug fixes, documentation generation, and other release-oriented tasks should go in this branch. Once it's ready to ship, the release gets a tag and is merged into `master`.
...
- `low priority`: These are lower priority than non-labeled items. Should be ignored unless all other issues/PRs have been addressed.
...
- Changes that are not related should be split into separate commits. This must be taken with the previous rule in mind to help determine what's related or not. Bugfixes that are prerequisites to your change but not the main point of the change should be made in separate commits that come first.
- Make sure changes are rebased on latest master. Ensure your code is up-to-date to prevent merge conflicts.

#### Commit Messages

Please follow the [git commit message standard](http://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html). See: http://whatthecommit.com for examples of what not to commit (refresh page).

The following emphasises or overrides where applicable:
...
- If you review a pull request and approve of everything in the code, merge the pull request into `master`, squashing all redundant, unnecessary or nonatomic commits.
...
- You should not submit a pull request when there are pull requests with the `review needed` label, unless they are assigned to someone else.
  - You should review this pull request before submiting your PR.
  - Note: If Travis builds are failing, you may mark a PR `in progress`
- If any of your PRs has an `in progress` flag you must address the comments, remove the `in progress` label and apply `review needed` label. Continue iterating this process in the branch is pulled in.
...
- If, after reviewing  the release branch, no further changes are necessary, the branch will be tagged with the appropriate version number and once pulled into master that tag will be automatically pushed live to production.
...
- Rebased on top of most recent tag of `master` , inserting your commit after the most recent tag of `master`.
